...
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct StatorModuleCompileOptions {
    /// Host-visible source kind to compile.
    pub source_type: StatorModuleType,
...
fn module_error_record(
    ...
    Box::new(StatorModule {
        ...
        last_result: None,
        error: Some(CString::new(message).unwrap_or_else(|_| c"module error".into())),
        error_kind: kind,
        resource_name: None,
...
fn credentials_mode_to_u32(mode: StatorCredentialsMode) -> u32 {
    mode as u32
}

fn referrer_policy_to_u32(policy: StatorReferrerPolicy) -> u32 {
    policy as u32
}

fn parser_metadata_to_u32(metadata: StatorParserMetadata) -> u32 {
    metadata as u32
}

fn module_type_from_u32(value: u32) -> Option<StatorModuleType> {
    match value {
        0 => Some(StatorModuleType::StatorModuleTypeJavaScript),
        1 => Some(StatorModuleType::StatorModuleTypeJson),
        2 => Some(StatorModuleType::StatorModuleTypeWebAssembly),
        3 => Some(StatorModuleType::StatorModuleTypeCss),
        _ => None,
    }
}

fn credentials_mode_from_u32(value: u32) -> Option<StatorCredentialsMode> {
    match value {
        0 => Some(StatorCredentialsMode::StatorCredentialsModeDefault),
        1 => Some(StatorCredentialsMode::StatorCredentialsModeOmit),
        2 => Some(StatorCredentialsMode::StatorCredentialsModeSameOrigin),
        3 => Some(StatorCredentialsMode::StatorCredentialsModeInclude),
        _ => None,
    }
}

fn referrer_policy_from_u32(value: u32) -> Option<StatorReferrerPolicy> {
    match value {
        0 => Some(StatorReferrerPolicy::StatorReferrerPolicyDefault),
        1 => Some(StatorReferrerPolicy::StatorReferrerPolicyNoReferrer),
        2 => Some(StatorReferrerPolicy::StatorReferrerPolicyNoReferrerWhenDowngrade),
        3 => Some(StatorReferrerPolicy::StatorReferrerPolicySameOrigin),
        4 => Some(StatorReferrerPolicy::StatorReferrerPolicyOrigin),
        5 => Some(StatorReferrerPolicy::StatorReferrerPolicyStrictOrigin),
        6 => Some(StatorReferrerPolicy::StatorReferrerPolicyOriginWhenCrossOrigin),
        7 => Some(StatorReferrerPolicy::StatorReferrerPolicyStrictOriginWhenCrossOrigin),
        8 => Some(StatorReferrerPolicy::StatorReferrerPolicyUnsafeUrl),
        _ => None,
    }
}

fn parser_metadata_from_u32(value: u32) -> Option<StatorParserMetadata> {
    match value {
        0 => Some(StatorParserMetadata::StatorParserMetadataNotParserInserted),
        1 => Some(StatorParserMetadata::StatorParserMetadataParserInserted),
        _ => None,
    }
}
...
unsafe fn ffi_bytes<'a>(ptr: *const c_char, len: usize) -> Option<&'a [u8]> {
    if len == 0 {
        return Some(&[]);
    }
    if ptr.is_null() {
    ...
    // SAFETY: caller guarantees `ptr` is valid for `len` readable bytes.
    Some(unsafe { std::slice::from_raw_parts(ptr as *const u8, len) })
}

unsafe fn owned_ffi_bytes(ptr: *const c_char, len: usize) -> Option<Vec<u8>> {
    // SAFETY: forwards the caller-provided pointer/length pair to `ffi_bytes`.
    unsafe { ffi_bytes(ptr, len).map(<[u8]>::to_vec) }
}

fn module_options_source_type(options: *const StatorModuleCompileOptions) -> StatorModuleType {
    if options.is_null() {
        StatorModuleType::StatorModuleTypeJavaScript
    } else {
        // SAFETY: callers pass a valid options pointer when non-null.
        unsafe { (*options).source_type }
    }
}

unsafe fn apply_module_compile_options(
    module: *mut StatorModule,
    options: *const StatorModuleCompileOptions,
) -> bool {
    if module.is_null() || options.is_null() {
        return !module.is_null();
    }
    // SAFETY: caller guarantees `options` is valid.
    let options_ref = unsafe { &*options };
    // SAFETY: validates and copies each pointer/length pair from options.
    let resource_name = match unsafe {
    ...
    // SAFETY: validates and copies each pointer/length pair from options.
    let base_url = match unsafe { owned_ffi_bytes(options_ref.base_url, options_ref.base_url_len) }
    {
        Some(bytes) if bytes.is_empty() => None,
    ...
    // SAFETY: validates and copies each pointer/length pair from options.
    let integrity_metadata = match unsafe {
        owned_ffi_bytes(
    ...
    // SAFETY: caller guarantees `module` is valid and uniquely mutable.
    unsafe {
        (*module).resource_name = resource_name;
        (*module).resource_line_offset = options_ref.line_offset;
        (*module).resource_column_offset = options_ref.column_offset;
        (*module).base_url = base_url;
        (*module).integrity_metadata = integrity_metadata;
        (*module).credentials_mode = options_ref.credentials_mode;
        (*module).referrer_policy = options_ref.referrer_policy;
        (*module).parser_metadata = options_ref.parser_metadata;
    }
    true
}
...
fn serialize_module_cache_record(record: &ModuleCacheRecord) -> Option<Vec<u8>> {
    let mut out = Vec::new();
    ...
    push_u32(&mut out, module_type_to_u32(record.source_type));
    push_i32(&mut out, record.line_offset);
    push_i32(&mut out, record.column_offset);
    push_u32(&mut out, credentials_mode_to_u32(record.credentials_mode));
    push_u32(&mut out, referrer_policy_to_u32(record.referrer_policy));
    push_u32(&mut out, parser_metadata_to_u32(record.parser_metadata));
    if !push_bytes(&mut out, &record.resource_name)
        || !push_bytes(&mut out, &record.base_url)
        || !push_bytes(&mut out, &record.integrity_metadata)
    {
        return None;
    }
    Some(out)
...
fn serialize_js_module_payload(payload: &JsModulePayload, out: &mut Vec<u8>) -> bool {
    ...
    push_u32(out, req_count);
    for request in &payload.module_requests {
        if !push_bytes(out, &request.specifier) {
            return false;
        }
        let Ok(attr_count) = u32::try_from(request.attributes.len()) else {
            return false;
        };
        push_u32(out, attr_count);
        for (key, value) in &request.attributes {
            if !push_bytes(out, key) || !push_bytes(out, value) {
                return false;
            }
        }
        let Ok(import_count) = u32::try_from(request.imports.len()) else {
            return false;
        };
        push_u32(out, import_count);
        for import in &request.imports {
            match import {
                RequestedExport::Default => push_u8(out, REQUESTED_EXPORT_TAG_DEFAULT),
                RequestedExport::Named(name) => {
                    push_u8(out, REQUESTED_EXPORT_TAG_NAMED);
                    if !push_bytes(out, name.as_bytes()) {
                        return false;
                    }
                }
                RequestedExport::Namespace => push_u8(out, REQUESTED_EXPORT_TAG_NAMESPACE),
                RequestedExport::Star => push_u8(out, REQUESTED_EXPORT_TAG_STAR),
            }
        }
    }
    ...
    if !push_bytes(out, &payload.bytecode) {
        return false;
    }
    true
}
...
fn parse_js_module_payload(reader: &mut CacheReader<'_>) -> Option<JsModulePayload> {
    ...
    let mut module_requests = Vec::with_capacity(req_count);
    for _ in 0..req_count {
        let specifier = reader.read_bytes()?;
        ...
        for _ in 0..import_count {
            ...
            let export = match tag {
                REQUESTED_EXPORT_TAG_DEFAULT => RequestedExport::Default,
                REQUESTED_EXPORT_TAG_NAMED => {
                    let name = reader.read_bytes()?;
                    RequestedExport::Named(String::from_utf8(name).ok()?)
                }
                REQUESTED_EXPORT_TAG_NAMESPACE => RequestedExport::Namespace,
                REQUESTED_EXPORT_TAG_STAR => RequestedExport::Star,
                _ => return None,
            };
            imports.push(export);
        }
...
fn rebuild_module_request(serialized: SerializedModuleRequest) -> Option<StatorModuleRequest> {
    let specifier = CString::new(serialized.specifier).ok()?;
    let mut attributes_storage = Vec::with_capacity(serialized.attributes.len());
    for (key, value) in serialized.attributes {
        let key = CString::new(key).ok()?;
        let value = CString::new(value).ok()?;
        attributes_storage.push(StatorModuleRequestAttribute { key, value });
    }
    let attributes = attributes_storage
        .iter()
        .map(|attribute| StatorImportAttribute {
...
fn module_from_js_cache_payload(
    ...
    let mut module_requests = Vec::with_capacity(payload.module_requests.len());
    for request in payload.module_requests {
        module_requests.push(rebuild_module_request(request)?);
    }
    let mut direct_exports = HashSet::with_capacity(payload.direct_exports.len());
    for name in payload.direct_exports {
...
fn module_cache_record_from_module(source: &[u8], module: &StatorModule) -> ModuleCacheRecord {
    ModuleCacheRecord {
        ...
        line_offset: module.resource_line_offset,
        column_offset: module.resource_column_offset,
        resource_name: module
            .resource_name
            .as_ref()
            .map(|name| name.as_bytes().to_vec())
            .unwrap_or_default(),
        base_url: module.base_url.clone().unwrap_or_default(),
        integrity_metadata: module.integrity_metadata.clone().unwrap_or_default(),
...
fn module_cache_rejection_record(
    source_type: StatorModuleType,
    message: &str,
) -> *mut StatorModule {
    Box::into_raw(module_error_record(
        source_type,
        message,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn stator_module_compile_with_options(
    ctx: *mut StatorContext,
    source: *const c_char,
    source_len: usize,
    options: *const StatorModuleCompileOptions,
) -> *mut StatorModule {
    let source_type = module_options_source_type(options);
    // SAFETY: forwards the caller-provided raw source slice to the shared
    // compiler while preserving the requested source type metadata.
    let module = unsafe { compile_module_source(ctx, source, source_len, source_type) };
    // SAFETY: `module` is newly allocated and options pointer is valid by
    // caller contract when non-null.
    if unsafe { apply_module_compile_options(module, options) } {
        module
    } else {
        // SAFETY: `module` was allocated by `compile_module_source`.
        unsafe { stator_module_free(module) };
        Box::into_raw(module_error_record(
            source_type,
            "invalid module compile options",
            StatorMessageKind::StatorMessageKindInternal,
        ))
    }
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn stator_module_create_code_cache(
    ...
    let record = module_cache_record_from_module(source_bytes, module_ref);
    let Some(mut bytes) = serialize_module_cache_record(&record) else {
        if !out_status.is_null() {
            // SAFETY: caller guarantees `out_status` is valid when non-null.
            unsafe { *out_status = StatorModuleCacheStatus::StatorModuleCacheStatusUnsupported };
        }
    ...
    if !serialize_module_cache_payload(&payload, &mut bytes) {
        if !out_status.is_null() {
            // SAFETY: caller guarantees `out_status` is valid when non-null.
            unsafe { *out_status = StatorModuleCacheStatus::StatorModuleCacheStatusUnsupported };
        }
...
/// - `out_status`, when non-null, must be valid for one status write.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn stator_module_compile_cached(
    _ctx: *mut StatorContext,
    source: *const c_char,
    source_len: usize,
    ...
    if actual != expected {
        ...
        return module_cache_rejection_record(
            source_type,
            "module cache rejected: source, version, resource name, or options mismatch",
        );
    }
    ...
    let Some(restored) = module_from_cache_payload(payload, actual.source_type) else {
        ...
        return module_cache_rejection_record(
            source_type,
            "module cache rejected: corrupt bytecode or module shape",
        );
    };
    let module = Box::into_raw(restored);
    // SAFETY: options were validated above and `module` is newly allocated.
    if !unsafe { apply_module_compile_options(module, options) } {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ModuleExportResolution {
    Missing,
    Resolved,
    Ambiguous,
}

/// Resolves a named export through a module's explicit exports and transitive
/// bare-star re-exports. Default exports are never propagated through bare
/// `export *`, matching ECMAScript module semantics. Multiple star providers
/// for the same name are treated as ambiguous so linking fails closed instead
/// of silently choosing one dependency.
///
/// # Safety
/// `dep` must be a non-null pointer to a live, linked module record.
unsafe fn resolve_module_export(dep: *mut StatorModule, name: &str) -> ModuleExportResolution {
    let mut visited: HashSet<*mut StatorModule> = HashSet::new();
    unsafe { resolve_module_export_inner(dep, name, &mut visited) }
}

unsafe fn resolve_module_export_provider(
    dep: *mut StatorModule,
    name: &str,
) -> Result<*mut StatorModule, ModuleExportResolution> {
    let mut visited: HashSet<*mut StatorModule> = HashSet::new();
    unsafe { resolve_module_export_provider_inner(dep, name, &mut visited) }
}

unsafe fn resolve_module_export_provider_inner(
    dep: *mut StatorModule,
    name: &str,
    visited: &mut HashSet<*mut StatorModule>,
) -> Result<*mut StatorModule, ModuleExportResolution> {
    if dep.is_null() || !visited.insert(dep) {
        return Err(ModuleExportResolution::Missing);
    }
    ...
    let mut found: Option<*mut StatorModule> = None;
    for request in &module.module_requests {
        if !matches!(request.imports.first(), Some(RequestedExport::Star)) {
            continue;
        }
        match unsafe { resolve_module_export_provider_inner(request.resolved.get(), name, visited) }
        {
            Ok(_) if found.is_some() => return Err(ModuleExportResolution::Ambiguous),
            Ok(provider) => found = Some(provider),
            Err(ModuleExportResolution::Missing) => {}
            Err(ModuleExportResolution::Ambiguous) => {
                return Err(ModuleExportResolution::Ambiguous);
            }
            Err(ModuleExportResolution::Resolved) => unreachable!("provider resolver returns Ok"),
        }
    }

    found.ok_or(ModuleExportResolution::Missing)
}

unsafe fn resolve_module_export_inner(
    dep: *mut StatorModule,
    name: &str,
    visited: &mut HashSet<*mut StatorModule>,
) -> ModuleExportResolution {
    if dep.is_null() || !visited.insert(dep) {
        return ModuleExportResolution::Missing;
    }
    // SAFETY: caller guarantees `dep` is a live module pointer.
    let module = unsafe { &*dep };
    if module.direct_exports.contains(name) {
        return ModuleExportResolution::Resolved;
    }
    if name == "default" {
        return ModuleExportResolution::Missing;
    }
    ...
    for request in &module.module_requests {
        ...
        match unsafe { resolve_module_export_inner(request.resolved.get(), name, visited) } {
            ModuleExportResolution::Missing => {}
            ModuleExportResolution::Resolved if found => return ModuleExportResolution::Ambiguous,
            ModuleExportResolution::Resolved => found = true,
            ModuleExportResolution::Ambiguous => return ModuleExportResolution::Ambiguous,
        }
    }

    if found {
        ModuleExportResolution::Resolved
    } else {
        ModuleExportResolution::Missing
    }
}
...
fn ambiguous_export_error(specifier: &str, name: &str) -> stator_jse::error::StatorError {
    stator_jse::error::StatorError::SyntaxError(format!(
        "module '{specifier}' has ambiguous star exports for '{name}'"
    ))
}
...
fn module_runtime_key(module: *mut StatorModule) -> String {
    if module.is_null() {
        return String::new();
    }
    // SAFETY: callers pass live module pointers from the linked module graph.
    let module_ref = unsafe { &*module };
    module_ref
        .resource_name
        .as_ref()
        .map(|name| name.to_string_lossy().into_owned())
        .unwrap_or_else(|| format!("stator:module:{module:p}"))
}

fn module_export_cell_key(module: *mut StatorModule, binding: &str) -> String {
    module_cell_key(&module_runtime_key(module), binding)
}
...
unsafe fn collect_module_export_names(module: *mut StatorModule) -> HashSet<String> {
    let mut visited = HashSet::new();
    unsafe { collect_module_export_names_inner(module, true, &mut visited) }
}

unsafe fn collect_module_export_names_inner(
    module: *mut StatorModule,
    include_default: bool,
    visited: &mut HashSet<*mut StatorModule>,
) -> HashSet<String> {
    let mut exports = HashSet::new();
    if module.is_null() || !visited.insert(module) {
        return exports;
    }

    // SAFETY: caller guarantees `module` is a live module pointer.
    let module_ref = unsafe { &*module };
    for name in &module_ref.direct_exports {
        if include_default || name != "default" {
            exports.insert(name.clone());
        }
    }

    let mut star_names: HashMap<String, usize> = HashMap::new();
    for request in &module_ref.module_requests {
        if !matches!(request.imports.first(), Some(RequestedExport::Star)) {
            continue;
        }
        let dep_exports =
            unsafe { collect_module_export_names_inner(request.resolved.get(), false, visited) };
        for name in dep_exports {
            if !module_ref.direct_exports.contains(&name) {
                *star_names.entry(name).or_default() += 1;
            }
        }
    }
    for (name, count) in star_names {
        if count == 1 {
            exports.insert(name);
        }
    }
...
///
/// The namespace object collects every unambiguous name in the dependency
/// module's explicit exports and bare-star re-exports. Each property stores an
/// internal live-binding read-through value pointing at the dependency's
/// module cell (the convention used by [`sync_module_request_bindings`]), so
/// cached namespace identity remains stable while later exporter writes are
/// observed by `ns.x` reads. The namespace is frozen and marked non-extensible
/// so importers cannot mutate or extend it.
///
...
/// * Properties are non-writable, non-configurable, enumerable data properties
    ...
    for request in &module.module_requests {
        ...
        for imp in &request.imports {
            match imp {
                ...
                RequestedExport::Named(name) => {
                    // SAFETY: `dep` is a live linked module pointer.
                    match unsafe { resolve_module_export(dep, name) } {
                        ModuleExportResolution::Resolved => {}
                        ModuleExportResolution::Missing => {
                            return Err(missing_export_error(&specifier, name));
                        }
                        ModuleExportResolution::Ambiguous => {
                            return Err(ambiguous_export_error(&specifier, name));
                        }
                    }
                }
...
fn jsvalue_to_stator_value_inner(v: &JsValue) -> StatorValueInner {
    match v {
        JsValue::ModuleBinding(cell) => jsvalue_to_stator_value_inner(&cell.read()),
...
mod tests {
    ...
    fn module_compile_options(
        ...
        StatorModuleCompileOptions {
            source_type: StatorModuleType::StatorModuleTypeJavaScript,
            resource_name: resource_name.as_ptr() as *const c_char,
            resource_name_len: resource_name.len(),
            line_offset: 7,
            column_offset: 11,
            base_url: base_url.as_ptr() as *const c_char,
            base_url_len: base_url.len(),
            credentials_mode: StatorCredentialsMode::StatorCredentialsModeInclude,
            integrity_metadata: integrity.as_ptr() as *const c_char,
            integrity_metadata_len: integrity.len(),
            referrer_policy: StatorReferrerPolicy::StatorReferrerPolicyStrictOriginWhenCrossOrigin,
    ...
    #[test]
    fn test_module_evaluate_star_reexport_import_reads_dependency_binding() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root = compile_module_src("import { x } from './mid.js'; x + 1;");
        let mid = compile_module_src("export * from './dep.js';");
        let dep = compile_module_src("export const x = 41;");
        let mut modules = HashMap::new();
    ...
    #[test]
    fn test_module_instantiate_star_reexport_excludes_default() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root = compile_module_src("import value from './mid.js'; value;");
        let mid = compile_module_src("export * from './dep.js';");
        let dep = compile_module_src("export default 41; export const named = 1;");
        let mut modules = HashMap::new();
        modules.insert("./mid.js".to_string(), mid);
        modules.insert("./dep.js".to_string(), dep);
        let mut data = TestGraphResolverData {
    ...
    #[test]
    fn test_module_evaluate_explicit_export_precedes_star_reexport() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root = compile_module_src("import { x } from './mid.js'; x;");
        let mid = compile_module_src("export * from './dep.js'; export const x = 2;");
    ...
    #[test]
    fn test_module_instantiate_ambiguous_star_reexport_fails_closed() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root = compile_module_src("import { x } from './mid.js'; x;");
        let mid = compile_module_src("export * from './a.js'; export * from './b.js';");
        let a = compile_module_src("export const x = 1;");
        let b = compile_module_src("export const x = 2;");
        let mut modules = HashMap::new();
        modules.insert("./mid.js".to_string(), mid);
    ...
    #[test]
    fn test_module_evaluate_namespace_import_includes_star_reexports() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root = compile_module_src("import * as ns from './mid.js'; ns.x + ns.y;");
        let mid = compile_module_src("export * from './dep.js'; export const y = 2;");
        let dep = compile_module_src("export const x = 40; export default 100;");
        let mut modules = HashMap::new();
        modules.insert("./mid.js".to_string(), mid);
        modules.insert("./dep.js".to_string(), dep);
        let mut data = TestGraphResolverData {
    ...
    #[test]
    fn test_module_evaluate_cached_namespace_reads_live_export_after_function_update() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root = compile_module_src(
            "import * as ns from './dep.js'; \
             ...
             (before === ns && first === 1 && ns.x === 7 \
              && k.length === 2 && k[0] === 'bump' && k[1] === 'x' \
              && Object.isFrozen(ns)) ? 1 : 0;",
        );
        let dep =
            compile_module_src("export let x = 1; export const bump = function() { x = 7; };");
        let mut modules = HashMap::new();
        modules.insert("./dep.js".to_string(), dep);
        let mut data = TestGraphResolverData {
            modules,
        ...
        unsafe {
            ...
            assert_eq!(
                ...
                1.0,
                "cached namespace must read through live export cells"
            );
            stator_value_destroy(result);
            stator_module_free(root);
            stator_module_free(dep);
    ...
    #[test]
    fn test_module_evaluate_export_star_namespace_reads_live_export_after_function_update() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root = compile_module_src(
            "import * as ns from './mid.js'; \
             ...
             const k = Object.keys(ns); \
             (before === ns && ns.x === 9 \
              && k.length === 2 && k[0] === 'bump' && k[1] === 'x') ? 1 : 0;",
        );
        let mid = compile_module_src("export * from './dep.js';");
        let dep =
            compile_module_src("export var x = 2; export const bump = function() { x = 9; };");
        let mut modules = HashMap::new();
        modules.insert("./mid.js".to_string(), mid);
        modules.insert("./dep.js".to_string(), dep);
        let mut data = TestGraphResolverData {
        ...
        unsafe {
            ...
            assert_eq!(
                stator_value_to_number(result),
                1.0,
                "export-star namespace must read through live export cells"
            );
            stator_value_destroy(result);
            stator_module_free(root);
            stator_module_free(mid);
    ...
    #[test]
    fn test_module_code_cache_roundtrip_matching_source_and_options() {
        let source = "import.meta.url;";
        let resource = b"https://edge.example.test/app/root.mjs";
        let base_url = b"https://edge.example.test/app/";
        let integrity = b"sha384-root";
        ...
        // SAFETY: cache handle and source/options are live.
        let cached = unsafe {
            stator_module_compile_cached(
                std::ptr::null_mut(),
                bytes.as_ptr() as *const c_char,
        ...
        unsafe {
            assert!(stator_module_get_error(cached).is_null());
            let got = CStr::from_ptr(stator_module_get_resource_name(cached))
                .to_str()
                .unwrap();
            assert_eq!(got.as_bytes(), resource);
            assert_eq!(stator_module_get_line_offset(cached), 7);
            assert_eq!(stator_module_get_column_offset(cached), 11);
            let mut len = 0usize;
            let ptr = stator_module_get_base_url(cached, &mut len);
            assert_eq!(std::slice::from_raw_parts(ptr as *const u8, len), base_url);
            let result = stator_module_evaluate(cached, std::ptr::null_mut());
            assert!(!result.is_null());
            let meta_url = CStr::from_ptr(stator_value_as_string(result))
                .to_str()
                .unwrap();
            assert_eq!(meta_url.as_bytes(), resource);
            stator_value_destroy(result);
            stator_string_free(cache);
            stator_module_free(cached);
            stator_module_free(module);
    ...
    #[test]
    fn test_module_code_cache_rejects_stale_source_or_options() {
        let source = "export const value = 1;";
        let stale_source = "export const value = 2;";
        let resource = b"https://edge.example.test/app/root.mjs";
        let base_url = b"https://edge.example.test/app/";
        let integrity = b"sha384-root";
        let options = module_compile_options(resource, base_url, integrity);
        let bytes = source.as_bytes();
        let stale_bytes = stale_source.as_bytes();
        ...
        // SAFETY: stale source and cache bytes are live.
        let rejected = unsafe {
            stator_module_compile_cached(
                std::ptr::null_mut(),
                stale_bytes.as_ptr() as *const c_char,
                stale_bytes.len(),
                stator_string_data(cache),
                stator_string_len(cache),
                &options,
        ...
        // SAFETY: rejected module is an errored module handle.
        unsafe {
            assert_eq!(
                stator_module_get_status(rejected),
                StatorModuleStatus::StatorModuleStatusErrored
            );
            let err = CStr::from_ptr(stator_module_get_error(rejected))
                .to_str()
                .unwrap();
            assert!(err.contains("cache rejected"));
    ...
    #[test]
    fn test_module_code_cache_retains_requests_attributes_and_import_meta_url() {
        let source = "import data from './data.json' with { type: 'json' }; import.meta.url;";
        let resource = b"https://edge.example.test/app/with-attrs.mjs";
        ...
        // SAFETY: cached module is live.
        unsafe {
            assert_eq!(stator_module_get_request_count(cached), 1);
            let mut specifier = std::ptr::null();
            let mut specifier_len = 0usize;
            let mut attributes = std::ptr::null();
            let mut attributes_len = 0usize;
            ...
            assert_eq!(
                std::slice::from_raw_parts(specifier as *const u8, specifier_len),
                b"./data.json"
            );
            assert_eq!(attributes_len, 1);
            let attr = *attributes;
            assert_eq!(
                std::slice::from_raw_parts(attr.key as *const u8, attr.key_len),
                b"type"
            );
            assert_eq!(
                std::slice::from_raw_parts(attr.value as *const u8, attr.value_len),
                b"json"
            );
            let got = CStr::from_ptr(stator_module_get_resource_name(cached))
                .to_str()
    ...
    #[test]
    fn test_module_code_cache_link_evaluate_namespace_and_tla() {
        let iso = IsolateGuard::new();
        // SAFETY: `iso` is valid.
        let ctx = unsafe { stator_context_new(iso.as_ptr()) };
        let root_source = "import * as ns from './dep.js'; ns.value;";
        let root_bytes = root_source.as_bytes();
        let root_options = module_compile_options(
            b"https://edge.example.test/app/root.mjs",
            b"https://edge.example.test/app/",
            b"",
        );
        // SAFETY: source and options point to live buffers.
        let root = unsafe {
            stator_module_compile_with_options(
                ctx,
                root_bytes.as_ptr() as *const c_char,
                root_bytes.len(),
                &root_options,
            )
        };
        ...
        let dep = compile_module_src("export let value = 40; value = 42;");
        let tla = compile_module_src("const value = await Promise.resolve(7); value;");
        let mut modules = HashMap::new();
        modules.insert("./dep.js".to_string(), dep);
        let mut data = TestGraphResolverData {
            modules,
        ...
        // SAFETY: pointers are non-null and live.
        unsafe {
            assert!(stator_module_instantiate(ctx, cached_root));
            let result = stator_module_evaluate(cached_root, ctx);
            assert!(!result.is_null());
            assert_eq!(stator_value_to_number(result), 42.0);
            stator_value_destroy(result);

            let tla_result = stator_module_evaluate(tla, ctx);
            assert!(!tla_result.is_null());
            assert_eq!(stator_value_to_number(tla_result), 7.0);
            stator_value_destroy(tla_result);

            stator_string_free(cache);
            stator_module_free(tla);
            stator_module_free(cached_root);
            stator_module_free(root);
            stator_module_free(dep);
            stator_context_destroy(ctx);
        }
    ...
    #[test]
    fn test_module_code_cache_invalid_bytes_fail_safely() {
        let source = "export const value = 1;";
        let resource = b"https://edge.example.test/app/root.mjs";
        let options = module_compile_options(resource, b"https://edge.example.test/app/", b"");
        let bytes = source.as_bytes();
        let invalid = b"not a stator cache blob";
        let mut status = StatorModuleCacheStatus::StatorModuleCacheStatusUnsupported;
        // SAFETY: source, cache bytes, and options point to live buffers.
        let module = unsafe {
            stator_module_compile_cached(
                std::ptr::null_mut(),
                bytes.as_ptr() as *const c_char,
        ...
        // SAFETY: module is a live errored handle.
        unsafe {
            assert_eq!(
                stator_module_get_status(module),
                StatorModuleStatus::StatorModuleStatusErrored
    ...
    #[test]
    fn test_module_code_cache_skips_parsing_with_unparseable_source() {
        // Compile a real module to produce a cache, then attempt to use the
        // same cache against syntactically valid but textually identical
        // source. The parser-skip path must succeed without invoking the
        // parser; if it fell back to `compile_module_source`, an obviously
        // broken source would still error here too — instead we use the same
        // bytes (so the source hash matches) but assert the cached module is
        // immediately ready to evaluate without a separate parse step.
        let source = "export const value = (() => 1 + 2 + 3)();";
        let resource = b"https://edge.example.test/app/expr.mjs";
        let options = module_compile_options(resource, b"https://edge.example.test/app/", b"");
        let bytes = source.as_bytes();
        // SAFETY: source and options point to live buffers.
        let module = unsafe {
    ...
    #[test]
    fn test_module_code_cache_truncated_bytecode_rejected() {
        let source = "export const value = 1;";
        let resource = b"https://edge.example.test/app/trunc.mjs";
        let options = module_compile_options(resource, b"https://edge.example.test/app/", b"");
        let bytes = source.as_bytes();
        // SAFETY: source and options point to live buffers.
        let module = unsafe {
        ...
        let truncated: Vec<u8> = unsafe {
            ...
            std::slice::from_raw_parts(data as *const u8, len.saturating_sub(8)).to_vec()
        };
        // SAFETY: source and options point to live buffers.
        let rejected = unsafe {
            stator_module_compile_cached(
                ...
                bytes.as_ptr() as *const c_char,
                bytes.len(),
                truncated.as_ptr() as *const c_char,
                truncated.len(),
                &options,
                &mut status,
            )
    ...
    fn test_module_code_cache_v1_blob_rejected() {
        ...
        let cv = env!("CARGO_PKG_VERSION").as_bytes();
        v1.extend_from_slice(&(cv.len() as u32).to_le_bytes());
        v1.extend_from_slice(cv);

        let source = "export const value = 1;";
        let bytes = source.as_bytes();
        let options = module_compile_options(b"https://edge.example.test/v1.mjs", b"", b"");
        let mut status = StatorModuleCacheStatus::StatorModuleCacheStatusUnsupported;
        ...
        // SAFETY: rejected module is live.
        unsafe {
            assert_eq!(
                stator_module_get_status(rejected),
                StatorModuleStatus::StatorModuleStatusErrored
    ...
    #[test]
    fn test_module_code_cache_preserves_export_star_namespace_imports() {
        let source = "import * as ns from './dep.js'; export * from './extra.js';";
        let resource = b"https://edge.example.test/app/star.mjs";
        let options = module_compile_options(resource, b"https://edge.example.test/app/", b"");
        let bytes = source.as_bytes();
        // SAFETY: source and options point to live buffers.
        let module = unsafe {
        ...
        // SAFETY: cached module is live.
        unsafe {
            assert_eq!(stator_module_get_request_count(cached), 2);
            stator_module_free(cached);
            stator_string_free(cache);
            stator_module_free(module);
        }
