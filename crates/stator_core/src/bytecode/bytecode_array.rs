...
pub struct BytecodeArray {
    ...
    is_module: bool,
    /// `true` if this bytecode belongs to a strict-mode function or script.
    is_strict: bool,
...
impl BytecodeArray {
    ...
    /// Mark this [`BytecodeArray`] as belonging to a strict-mode function or
    /// script (one whose prologue contains `"use strict"`).
    pub fn with_strict_flag(mut self, flag: bool) -> Self {
        self.is_strict = flag;
        self
    }

    /// Returns `true` if this bytecode belongs to a strict-mode function or
    /// script.
...
mod tests {
    ...
    fn test_strict_flag() {
        let array = make_simple_array();
        assert!(!array.is_strict(), "default should not be strict");

        let strict_array = make_simple_array().with_strict_flag(true);
        assert!(strict_array.is_strict(), "should be strict after flag set");
    }
}
