...
impl FunctionCompiler {
    ...
    ///
    /// The generated bytecode emits the try block, a jump past the catch block
    /// on normal completion, the catch block (if present), and finally inline
    /// after both paths.
    ///
    /// **Note**: Full VM-level exception dispatch requires a handler table
    /// that maps bytecode ranges to catch-handler addresses.  That table is
    /// not yet emitted; this method only produces the structural sequence.
    fn compile_try(&mut self, s: &crate::parser::ast::TryStmt) -> StatorResult<()> {
        ...
        // On normal exit, jump past the catch block.
        self.emit_jump(Opcode::Jump, after_catch_label);

        // Compile catch block.
        if let Some(handler) = &s.handler {
            // Accumulator holds the caught exception here (from runtime).
        ...
        // Compile finally block (executed on all paths).
