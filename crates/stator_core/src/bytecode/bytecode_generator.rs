        ...
        match &declarator.id {
            Pat::Ident(ident) => {
                if self.is_eval_scope {
                    ...
                    // Top-level `var` declarations live exclusively in the
                    // global environment (no local register).  This ensures
                    // that nested function callbacks that write to the same
                    // name via StaGlobal share a single storage location
                    // with the outer script, making closure-like mutation
                    // through callbacks (e.g. forEach) observable.
                    if let Some(init) = &declarator.init {
                        self.compile_expr(init)?;
                    } else {
                        self.emit(Instruction::new_unchecked(Opcode::LdaUndefined, vec![]));
