            ...
            match key {
                ...
                "__lookupGetter__" => {
                    ...
                    return JsValue::NativeFunction(Rc::new(move |args| {
                        ...
                        let mut getter_key: Option<String> = None;
                        for _ in 0..256 {
                            if let JsValue::PlainObject(ref m) = cur {
                                let b = m.borrow();
                                if b.has_accessors {
                                    let gk =
                                        getter_key.get_or_insert_with(|| format!("__get_{prop}__"));
                                    if let Some(g) = b.get(gk.as_str()) {
                                        return Ok(g.clone());
                                    }
                                }
                ...
                "__lookupSetter__" => {
                    ...
                    return JsValue::NativeFunction(Rc::new(move |args| {
                        ...
                        let mut cur = JsValue::PlainObject(Rc::clone(&map));
                        let mut setter_key: Option<String> = None;
                        for _ in 0..256 {
                            if let JsValue::PlainObject(ref m) = cur {
                                let b = m.borrow();
                                if b.has_accessors {
                                    let sk =
                                        setter_key.get_or_insert_with(|| format!("__set_{prop}__"));
                                    if let Some(s) = b.get(sk.as_str()) {
                                        return Ok(s.clone());
                                    }
                                }
                                match b.get(INTERNAL_PROTO_PROPERTY_KEY) {
