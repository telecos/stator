...
impl InterpreterFrame {
    ...
    pub(super) fn load_global(&mut self, name: &str) -> StatorResult<JsValue> {
        if let Some(val) = self.global_cache_get(name).cloned() {
            if name == "this" && val == JsValue::TheHole {
        ...
        let val = self
            ...
            .cloned()
            .unwrap_or(JsValue::Undefined);
        self.global_cache_put(name, val.clone());
        if name == "this" && val == JsValue::TheHole {
            return Err(StatorError::ReferenceError(
                "Must call super constructor in derived class before accessing 'this'".into(),
            ));
                                ...
                                | Opcode::TestGreaterThanOrEqual => {
                                    ...
                                    if let Some(next_instr) = instructions.get(pc) {
                                        match next_instr.opcode {
                                            Opcode::JumpIfTrue => {
                                                if result {
                                                    // SAFETY: The looked-ahead jump has a
                                                    // pre-computed target and `pc + 1` is the
                                                    // post-jump program counter.
                                                    pc = unsafe {
...
///
/// The bytecode generator guarantees `instr.operands[idx]` is
/// [`Operand::ConstantPoolIdx`] for the calling opcode. `idx` must be in
/// bounds.
#[inline(always)]
