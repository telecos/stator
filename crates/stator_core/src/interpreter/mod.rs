...
/// Returns `true` when `map` exposes an own property named `key`.
///
...
/// Ordinary `[[SetPrototypeOf]]` for `PlainObject`-backed objects.
pub(crate) fn ordinary_set_prototype_of(obj: &JsValue, proto: JsValue) -> StatorResult<()> {
...
///
/// Stores the value found by walking the prototype chain, guarded by the
/// receiver's layout and the prototype-chain generation counter.  When both
...
impl InterpreterFrame {
    ...
    /// Write `value` to a validated register operand without bounds checks.
    ///
    /// # Safety
    ///
    /// The caller must ensure `reg` came from validated bytecode and maps to a
    /// live register in this frame.
    #[inline(always)]
    unsafe fn write_reg_unchecked(&mut self, reg: u32, value: JsValue) {
        let signed = reg as i32;
        let idx = if signed >= 0 {
            self.bytecode_array.parameter_count() as usize + signed as usize
        } else {
...
impl Interpreter {
    ...
    fn run_dispatch(frame: &mut InterpreterFrame) -> StatorResult<JsValue> {
        ...
        'tail_call: loop {
            ...
            // Hoist the hottest frame fields into local variables so LLVM
            // can keep them in machine registers across the entire dispatch
            // loop.  Written back to `frame` before any cold-path call and
            // at every exit point.
            let mut pc = frame.pc;
            ...
            'dispatch: loop {
                ...
                match instr.opcode {
                            ...
                            if let JsValue::PlainObject(map) = obj_ref {
                                    ...
                                    .and_then(|cache| cache.probe(slot, layout))
                                {
                                    ...
                                    if let Some(ConstantPoolEntry::String(key_str)) =
                                        ...
                                        && pm.matches_key_at_offset(offset, key_str.as_str())
                    ...
                    // inline; falls back to the dispatch table for edge
                    // cases (PlainObject.__call__, NativeFunction, etc.).
                    Opcode::CallUndefinedReceiver0 => {
                        let reg = unsafe { operand_reg_unchecked(instr, 0) };
                        let callee = unsafe { frame.read_reg_unchecked(reg) };
                        if let JsValue::Function(ba) = callee
...
/// [`Operand::Register`] for the calling opcode.  `idx` must be in bounds.
#[inline(always)]
unsafe fn operand_reg_unchecked(
...
/// Extract an `Immediate(i32)` operand without checking the variant tag.
///
/// # Safety
///
/// The bytecode generator guarantees `instr.operands[idx]` is
/// [`Operand::Immediate`] for the calling opcode.  `idx` must be in bounds.
#[inline(always)]
unsafe fn operand_imm_unchecked(
    instr: &crate::bytecode::bytecodes::Instruction,
    idx: usize,
) -> i32 {
...
/// Resolve a pre-computed jump target without error checks.
///
/// # Safety
///
/// The caller must ensure `frame_pc > 0` and that `jump_targets[frame_pc - 1]`
/// contains a valid `Some(target)`.  These invariants hold for all jump opcodes
/// emitted by the bytecode compiler, which always populates jump targets for
/// every jump instruction during pre-decoding.
#[inline(always)]
unsafe fn resolve_jump_unchecked(frame_pc: usize, jump_targets: &[Option<usize>]) -> usize {
    // SAFETY: frame_pc was incremented past the instruction, so frame_pc - 1
    // is the index of the jump instruction.  The bytecode compiler guarantees
    // a valid target exists.
    let idx = frame_pc - 1;
    match *unsafe { jump_targets.get_unchecked(idx) } {
        Some(target) => target,
        // SAFETY: The bytecode compiler guarantees all jump instructions have
        // pre-computed targets populated during decode.
...
/// Extracts the prototype value from a [`PropertyMap`], checking both the
/// internal prototype key and the user-visible `__proto__` key.
#[inline]
...
///
/// Returns `None` if the key is not found before the chain ends or exceeds
...
mod tests {
    ...
    fn test_hot_registers_initial_state() {
        ...
        // All registers start as invalid (fall through to real register file).
        for i in 0..3 {
            assert_eq!(hr.try_read(i), None);
        }
    }
