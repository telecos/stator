...
/// Resolve the constructor's `.prototype` value, using the per-function
/// cache on `ba` when available.  Falls back to the normal `proto_lookup`
/// path and populates the cache for future calls.
