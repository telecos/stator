//! `stator_jse_test262` — Test262 conformance runner for the Stator engine.
//!
                    ...
                    let result = match &func {
                        ...
                        JsValue::PlainObject(map) => {
                            if let Some(call_fn) = map.borrow().get("__call__").cloned() {
                                stator_jse::interpreter::dispatch_call_value(&call_fn, vec![])
                            } else {
        ...
        match args[i].as_str() {
            ...
            "--threshold" => {
                ...
                if i < args.len() {
                    match args[i].parse::<f64>() {
                        ...
                        Err(_) => {
                            eprintln!(
                                "stator_jse_test262: invalid --threshold value '{}', using 0.0",
                                args[i]
                            );
                        }
                    }
            ...
            other if other.starts_with("--threshold=") => {
                ...
                match raw.parse::<f64>() {
                    ...
                    Err(_) => {
                        eprintln!(
                            "stator_jse_test262: invalid --threshold value '{raw}', using 0.0"
                        );
                    }
                }
            }
...
fn main_inner() {
    ...
    match stacker::remaining_stack() {
        Some(remaining) => eprintln!(
            "stator_jse_test262: remaining stack at startup: {} bytes ({:.1} MiB)",
            remaining,
            remaining as f64 / (1024.0 * 1024.0)
        ),
        None => eprintln!("stator_jse_test262: remaining_stack() returned None (unknown platform)"),
    }
    ...
    let base_dir = match cli.test262_dir {
        Some(d) => d,
        None => {
            eprintln!("stator_jse_test262: no test262 directory specified.");
    ...
    if !base_dir.is_dir() {
        eprintln!(
            "stator_jse_test262: test262 directory not found: {}",
            base_dir.display()
        );
        std::process::exit(1);
    }
    ...
    if !test_dir.is_dir() {
        eprintln!(
            "stator_jse_test262: 'test' subdirectory not found inside: {}",
            base_dir.display()
        );
        std::process::exit(1);
    }
    ...
    if let Err(e) = collect_tests(&test_dir, &test_dir, &mut test_files) {
        eprintln!("stator_jse_test262: error reading test directory: {e}");
        std::process::exit(1);
    }
    ...
    println!("stator_jse_test262: running {total} tests …");
        ...
        && let Ok(contents) = std::fs::read_to_string(pf)
    {
        ...
        if parts.len() >= 4 {
            ...
            println!("stator_jse_test262: resuming with pass={pass} fail={fail} skip={skip}");
        }
    }
    ...
    println!(
        "stator_jse_test262: globals initialized in {:.1}s",
    ...
    if cli.skip_first > 0 {
        println!(
            "stator_jse_test262: skipping first {} tests (crash recovery)",
            cli.skip_first
        );
    }
...
mod tests {
    ...
    fn test_collect_tests_empty_dir() {
        let tmp = std::env::temp_dir().join("stator_jse_test262_empty_collect_test");
    ...
    fn test_collect_tests_finds_js() {
        let tmp = std::env::temp_dir().join("stator_jse_test262_collect_test_finds_js");
        let _ = std::fs::create_dir_all(&tmp);
    ...
    #[test]
    fn test_collect_tests_keeps_async_generator_function_allowlist() {
        let tmp =
        ...
        .unwrap();
        std::fs::write(
            async_gen_dir.join("AsyncGeneratorFunctionPrototype-is-extensible.js"),
            "AsyncGeneratorFunction",
        )
        .unwrap();
        std::fs::write(
