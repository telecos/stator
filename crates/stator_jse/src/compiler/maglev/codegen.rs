...
//! };
//! use stator_jse::compiler::maglev::codegen::compile;
//!
...
impl<'a> MaglevCodegen<'a> {
    ...
    /// ; (Win64 only) push rdi ; push rsi   ; ABI-extra callee-saved
    /// mov   r14, <arg0> ; r14 = regs argument (RDI on SysV, RCX on Win64)
    /// ```
    ///
    /// On ABIs that mark additional registers as non-volatile (Win64 adds
    /// `RDI` and `RSI` to the SysV callee-saved set), those registers are
    /// pushed immediately after the fixed callee-saved block.  The slice
    /// is empty under SysV, so generated bytes are unchanged on Unix.
    /// `extra_entry_callee_saved()` always has even cardinality, so the
    /// existing `RSP ≡ 8 mod 16` invariant after the prologue's fixed
    /// pushes is preserved.
    fn emit_prologue(&mut self) {
        ...
        // Save ABI-specific extra entry callee-saved registers (empty on
        // SysV; `[RDI, RSI]` on Win64).  Pushed in declaration order; the
        // epilogue paths pop in reverse.
        for &reg in crate::compiler::abi_x64::NATIVE_ABI.extra_entry_callee_saved() {
            self.masm.push(reg);
        }
    ...
    ///
    /// ```text
    /// ; (Win64 only) pop rsi ; pop rdi    ; reverse of prologue extras
    /// pop  r15
    /// pop  r12
    /// pop  r13
    /// pop  r14
    ...
    fn emit_normal_epilogue(&mut self) {
        ...
        // Restore ABI-specific extra entry callee-saved registers in
        // reverse push order (no-op on SysV).
        for &reg in crate::compiler::abi_x64::NATIVE_ABI
            .extra_entry_callee_saved()
            .iter()
            .rev()
        {
            self.masm.pop(reg);
    ...
    fn emit_deopt_epilogue(&mut self) {
        ...
        // Restore ABI-specific extra entry callee-saved registers in
        // reverse push order (no-op on SysV); mirrors `emit_normal_epilogue`.
        for &reg in crate::compiler::abi_x64::NATIVE_ABI
            .extra_entry_callee_saved()
            .iter()
            .rev()
    ...
    fn emit_value_node(&mut self, id: NodeId, node: &ValueNode) {
        match node {
            ...
            ValueNode::Int32Add { left, right } => {
                if self.fused_skip.contains(&id) {
                    // Subsumed by a fused add-sub pair — skip emission.
            ...
            ValueNode::LoadGlobal {
                ...
                if let Some(off) = self.promoted_global_offset(*name) {
                    ...
                    // feed only into arithmetic, matching the promoted path.
                    if self.smi_guarded.contains(&id) && !self.i32_range.contains(&id) {
            ...
            // Specialized push call — uses 2-arg jit_runtime_array_push
    ...
    fn emit_inline_generic_add(&mut self, id: NodeId, left: NodeId, right: NodeId) {
        ...
        //
        // All smi_guarded sources have Smi guards at their definition site
        // (LoadNamedGeneric, LoadGlobal), so undefined values never reach
        // this point — no per-operation undef guard needed.
    ...
    fn emit_inline_generic_mul(&mut self, id: NodeId, left: NodeId, right: NodeId) {
        if self.i32_range.contains(&left) && self.i32_range.contains(&right) {
            ...
            // Non-constant: IMUL32-register + JO + MOVSXD.
            match self.alloc.location(id) {
                Some(Location::Register(n)) => {
                    let dst = phys_reg(n);
                    let right_in_dst = left != right
    ...
    fn emit_inline_load_named(
        ...
        // The handle comparison is sufficient — if the handle matches,
        // the object identity is confirmed and the cached property
        // offset is still valid.  Shape changes invalidate the IC via
        // the IC invalidation logic, so a separate shape_id check is
        // redundant on the hit path.
        self.masm.cmp_rm(Reg64::R11, Reg64::Rbp, ic_off_handle);
        self.masm.jne(&mut slow_label);

        // Handle matches → property at same offset.
        ...
        if is_smi_specialized {
            ...
            //
            // Instruction count on Smi IC hit: 8 (vs 12 before).
    ...
    fn emit_direct_call_0(&mut self, id: NodeId, callee: NodeId) {
        ...
        // Load BA ptr first so both loads can issue in parallel.
        ...
        // Unrolled zeroing: always zero slots 0–3 (32 bytes) with
        // direct RSP-relative stores.  For closures with ≤4 slots
        // (the common case) this replaces the counted loop entirely.
        self.masm.xor_rr(Reg64::Rax, Reg64::Rax);
        ...
        // For closures with > 4 register slots, fall into a loop
        // that zeroes the remaining slots starting at offset 32.
        self.masm
    ...
    fn compute_narrow_int32(
        ...
        // mark Phi nodes AND Int32 arithmetic on the backward path as
        // narrow-transparent so their MOVSXD can be elided.
    ...
    fn compute_fused_add_sub(&mut self) {
        ...
        for block in self.graph.blocks() {
            for (sub_id, sub_node) in &block.nodes {
                ...
                // or rematerialisable).  Skip if both are constants — the
