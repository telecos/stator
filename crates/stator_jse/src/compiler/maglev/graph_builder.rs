...
mod tests {
    ...
    /// the builder should emit a `LoadKeyedGeneric` node.
    #[test]
    fn test_keyed_load_monomorphic_builds() {
        let instrs = vec![
            Instruction::new_unchecked(Opcode::LdaSmi, vec![Operand::Immediate(0)]),
        ...
        assert!(
            !graph.is_degenerate(),
            "keyed load monomorphic should produce a non-degenerate graph"
        );
    }

    /// Keyed load with uninitialized feedback should build (produces generic node).
    #[test]
    fn test_keyed_load_uninitialized_builds() {
        let instrs = vec![
            Instruction::new_unchecked(Opcode::LdaSmi, vec![Operand::Immediate(0)]),
            Instruction::new_unchecked(Opcode::Star, vec![Operand::Register(1)]),
            Instruction::new_unchecked(Opcode::LdaSmi, vec![Operand::Immediate(0)]),
        ...
        // Uninitialized feedback still produces a graph (generic keyed load).
        let _graph = GraphBuilder::build(&arr, &fv).unwrap();
    }

    /// Keyed store with monomorphic feedback should build successfully.
    #[test]
    fn test_keyed_store_monomorphic_builds() {
        let instrs = vec![
            Instruction::new_unchecked(Opcode::LdaSmi, vec![Operand::Immediate(42)]),
            Instruction::new_unchecked(
    ...
    /// Keyed load with megamorphic feedback should build (generic path).
    #[test]
    fn test_keyed_load_megamorphic_builds() {
        let instrs = vec![
            Instruction::new_unchecked(Opcode::LdaSmi, vec![Operand::Immediate(0)]),
            Instruction::new_unchecked(Opcode::Star, vec![Operand::Register(1)]),
            Instruction::new_unchecked(Opcode::LdaSmi, vec![Operand::Immediate(0)]),
    ...
    /// Regression test: sieve_primes benchmark builds a Maglev graph.
    #[test]
    fn test_sieve_benchmark_builds() {
        use crate::bytecode::bytecode_generator::BytecodeGenerator;
        use crate::parser::recursive_descent;
        ...
        assert!(
            !graph.is_degenerate(),
            "sieve graph should be non-degenerate"
        );
    }
    ...
    /// Array literals with keyed access (a[i]) should build a Maglev graph.
    #[test]
    fn test_array_literal_with_keyed_access_builds() {
        use crate::bytecode::bytecode_generator::BytecodeGenerator;
        use crate::parser::recursive_descent;
        ...
        assert!(
            !graph.is_degenerate(),
            "array literal with keyed access should produce a non-degenerate graph"
        );
    }
