...
impl<'a> BaselineCompiler<'a> {
    ...
    /// pushed immediately after the fixed callee-saved block — this set is
    /// empty under SysV, so the generated bytes are unchanged on Unix.
    /// The set always has an even cardinality (`{}` on SysV, `{RDI, RSI}`
    /// on Win64), so the existing 16-byte stack-alignment math is
    /// preserved.
    ///
    /// ```text
    /// push rbp
    ...
    /// mov  r14, <arg0> ; register-file pointer  (RDI on SysV, RCX on Win64)
    /// mov  rbx, <arg1> ; closure context        (RSI on SysV, RDX on Win64)
    ...
    fn emit_prologue(&mut self) {
        ...
        // epilogue pops in reverse.  The slice always has even length so
        // 16-byte stack alignment is preserved across this block.
