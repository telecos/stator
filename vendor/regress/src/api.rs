...
impl Match {
    ...
    pub fn named_group(&self, name: &str) -> Option<Range> {
        ...
        for (idx, group_name) in self.group_names.iter().enumerate() {
            ...
            let range = self.captures[idx].clone();
            if range.is_some() {
                return range;
            }
            if fallback.is_none() {
...
impl<'m> Iterator for NamedGroups<'m> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while self.next_group_idx < end {
            let idx = self.next_group_idx;
            self.next_group_idx += 1;
            let name = self.mat.group_names[idx].as_ref();
            if name.is_empty() || !self.yielded.insert(name) {
                continue;
            }

            let mut range = self.mat.captures[idx].clone();
            let mut scan = idx + 1;
            while scan < end {
                if self.mat.group_names[scan].as_ref() == name {
                    let candidate = self.mat.captures[scan].clone();
                    if range.is_none() && candidate.is_some() {
                        range = candidate;
                    }
                }
                scan += 1;
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let mut seen = self.yielded.clone();
