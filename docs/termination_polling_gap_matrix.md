...
| RegExp wrapper (`crates\stator_jse\src\objects\regexp.rs`) | global match/replace/split/matchAll loops | No | Wrapper loops and `regress` engine calls do not poll. Pathological or very large regexp work can remain unbounded until the match call returns. |
