...
### 3.0 DOM wrapper call/construct handlers

DOM wrappers are ordinary objects unless Edge explicitly installs a `StatorDomCallableHandler` with `stator_dom_object_wrap_install_callable_handler`.
The call handler models V8-style `CallAsFunctionHandler`; the construct handler models a constructor callback. Either field may be null, but both null is rejected.

Handler contract:
- Calls receive a borrowed receiver value, a borrowed argument pointer array, opaque embedder data, and an owned `StatorValue** out` result slot.
- Constructs receive a borrowed `new_target` value plus the same argument and result channel.
- `StatorStatusOk` succeeds; a null `*out` means JavaScript `undefined`.
- `StatorStatusFalse` rejects the invocation, `StatorStatusException` records or preserves a pending exception, and invalid arguments fail closed.
- Invalidated/destroyed wrappers and access-check denial never invoke the callback.

Limitations: callable state is per wrapper instance (not a reusable template object), generic `HasInstance`/signature validation is still represented only by class-id/access-check hooks, and constructor primitive returns follow the engine's current ordinary-constructor fallback behavior.
