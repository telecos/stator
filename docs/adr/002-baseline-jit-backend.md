...
(`crates/stator_core/src/interpreter/mod.rs`).  The interpreter is correct and
...
* [`BytecodeArray`][bca] exposes a decoded `Vec<Instruction>` through
  `instructions()`.
* The inline-cache (`crates/stator_core/src/ic/`) and feedback-vector
...
[bca]: ../../crates/stator_core/src/bytecode/bytecode_array.rs
[op]:  ../../crates/stator_core/src/bytecode/bytecodes.rs
...
* **Zero new dependencies.**  The entire assembler lives inside `stator_core`
...
* **Positive:** The custom assembler stays inside `stator_core` with no
